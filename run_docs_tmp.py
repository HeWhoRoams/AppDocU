...
def iterate(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose logging"),
    run_llm: bool = typer.Option(True, "--llm/--no-llm", help="Run LLM generation each iteration"),
):
    ...
    def compute_roi() -> dict:
        ...
        # Coverage: if manifest present, compare nodes to code files in manifest
    ...
    def append_history(current: dict):
        ...
        if runs:
            ...
            try:
                ...
                current["diff"] = {"delta_code_nodes": d_nodes, "delta_edges": d_edges}
            except Exception:
                current["diff"] = {}
        runs.append(current)
        hist["runs"] = runs
    ...
    def do_iteration(iter_idx: int):
        ...
        # Present ROI summary
        typer.echo("\nROI Summary:")
        typer.echo(f"  Coverage: {entry['coverage'].get('coverage_pct',0)}% ({entry['coverage'].get('code_nodes',0)}/{entry['coverage'].get('manifest_code',0)})")
        typer.echo(f"  Completeness: {entry['completeness'].get('completeness_pct',0)}%")
        typer.echo(f"  Failure rate: {entry['completeness'].get('failure_rate_pct',0)}%")
        if entry.get('diff'):
            typer.echo(f"  Delta code nodes: {entry['diff'].get('delta_code_nodes',0)}")
            typer.echo(f"  Delta edges: {entry['diff'].get('delta_edges',0)}")
