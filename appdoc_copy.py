    ...
    def run_pass_1_discovery(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Fallback: enumerate files and create manifest without full normalization
                self.logger.warning(f"Preprocessing failed, falling back to enumeration: {pe}")
                try:
                    from appdocu_preprocessor.file_enumerator import FileEnumerator
                    enumerator = FileEnumerator(self.target_path)
                    file_infos = enumerator.enumerate_files()
                    manifest_path = enumerator.write_file_manifest(file_infos, self.output_dir)
                    ...
                    self.logger.error(f"Enumeration fallback failed: {ee}")
                    raise
